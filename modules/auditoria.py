...
def render_auditoria(results):
    ...
    with t3:
        ...
        # Integración con Tech Stack (Corregido)
        if 'tech_results' in st.session_state and st.session_state.tech_results:
            for t in st.session_state.tech_results:
                summary_list.append({"Módulo": "Tech Stack", "Hallazgo": f"Software detectado: {t.get('Valor')}", "Severidad": "Baja", "Impacto": "Enumeración"})

        # Integración con Seguridad de Formularios (Corregido: Usa 'Protección CSRF')
        ...
        # Integración con Fuzzer (Corregido: Usa 'fuzzer_results')
        if 'fuzzer_results' in st.session_state and st.session_state.fuzzer_results:
            for entry in st.session_state.fuzzer_results:
                summary_list.append({"Módulo": "Fuzzer", "Hallazgo": f"Directorio hallado: {entry['Ruta']}", "Severidad": "Media", "Impacto": "Exposición de Archivos"})
        ...
        if summary_list:
            ...
            st.info("⚠️ Ejecuta los módulos individuales (Formularios, Fuzzer, etc.) para que sus hallazgos aparezcan aquí.")
